...
function drawGrapheText(x, y, id = null, value, s_color) {
  let graphe = d3.select("svg").append("g").attr("id", "text_graphe");
...
function drawCircle(container, x, y, r, color = null, id = null) {
  let ele = d3.select("svg").append("g");
...
function show_choosed_bar() {
  let last_g = colors.length - 1;
  let last_color = colors[colors.length - 1];
  let p_color = d3.select("#button_show_color").attr("fill");
...
function toggle_color(el) {
  let element = d3.select(el);
  let color = d3.select(el).attr("fill");
  if (color == "white") {
...
function close_palet() {
  let el = d3.selectAll(".graphe_container");
    ...
    for (let i = 0; i < this.color.length; i++) {
      this.container
        ...
        .on("dblclick", function () {
          let el = d3.select(this);
          let index = el.attr("data_color_index");
